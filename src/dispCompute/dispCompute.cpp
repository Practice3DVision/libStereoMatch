...
namespace libSM {
     ...
     */
    bool uniqueCheck(const float majorMinCost, const float minorMinCost, const float uniquenessRatio = 0.8f) {
        return (minorMinCost - majorMinCost) > majorMinCost * (1.f - uniquenessRatio);
    }
     ...
     * @return false            not passed
     */
    bool lrCheck(const float* ptrCostMap, const int lx, const int rx, const int minDisp, const int maxDisp, const int cols, const int lrCheckThreshod = 1) {
     ...
     */
    float subpixelFitting(const float* ptrCostMap, const int lx, const int disp, const float minCost, const int dispRange) {
    ...
    void winnerTakesAll(const Mat& costMap, Mat& dispMap, const DispComputeParams params) {
...
#pragma omp parallel for schedule(dynamic) default(shared)
        for (int i = 0; i < costMap.rows; ++i) {
            ...
            for (int j = 0; j < costMap.cols; ++j) {
                ...
                if(params.uniqueCheck && !uniqueCheck(majorMinCost, minorMinCost, params.uniquenessRatio)) {
                    ptrDispMap[j] = 0.f;
                    continue;
                }

                if(params.lrCheck && !lrCheck(ptrCostMap, j, j - (majorDisp + params.minDisp), params.minDisp, params.maxDisp, costMap.cols, params.lrCheckThreshod)) {
                    ptrDispMap[j] = 0.f;
                    continue;
                }

                if(params.subpixelFitting && (majorDisp != 0 && majorDisp != costMap.channels() - 1)) {
                    ptrDispMap[j] = subpixelFitting(ptrCostMap, j, majorDisp, majorMinCost, costMap.channels()) + params.minDisp;
                }
                else {
